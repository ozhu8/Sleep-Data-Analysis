...
except ImportError:  # pragma: no cover - fallback for older scikit-image versions
    from skimage.future import graph as skgraph


def edge_weight(graph_rag: skgraph.RAG, graph_edge: tuple, graph_data: np.ndarray) -> float:
    """
...
def merge_weight(graph_rag: skgraph.RAG, src: int, dst: int, neighbor: int,
...
def merge_regions(graph_rag: skgraph.RAG, src: int, dst: int):
    """
...
def merge_nodes(graph_rag: skgraph.RAG, src: int, dst: int, graph_data: np.ndarray):
...
def trim_region(graph_rag: skgraph.RAG, labels_merged: np.ndarray, graph_data: np.ndarray, region_num: int,
                trim_volume: float):
...
def detect_tfpeaks(segment_data: np.ndarray, start_time=0, d_time=1, d_freq=1, merge_thresh=8, max_merges=np.inf,
    ...
    RAG = skgraph.RAG(join_labels, connectivity=2)
